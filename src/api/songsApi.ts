...
async function fetchSongsFromBPMApi(bpm: number, limit: number): Promise<string[]> {
  ...
  return response.data.tempo.map((song: { song_title: string }) => song.song_title);
}

async function searchOnSpotify(title: string): Promise<Song | null> {
  try {
    const sdk = await spotify;
    const result = await sdk.search(title, ["track"]);
...
export async function fetchSongsByBPM(bpm: number): Promise<Song[]> {
  try {
    const titles = await fetchSongsFromBPMApi(bpm, 10);
    const tracks: Song[] = [];

    for (const title of titles) {
      const result = await searchOnSpotify(title);
      if (result) {
        tracks.push(result);
      }
    }
...
export async function mockFetchSongsByBPM(bpm: number): Promise<Song[]> {
